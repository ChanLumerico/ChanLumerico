...
(() => {
  ...
  const stylesheet =
    ...
    '  background:#fff;box-shadow:0 0 0 1.5px var(--ac,#0071e3),0 1px 3px rgba(0,0,0,.3);' +
    ...
    // Reframing lifts the slot into a centred floating panel over a dimmed,
    // blurred page — the crop guide is the same 1:1 mask, just larger.
    ':host([data-reframe]){position:fixed;inset:0;margin:auto;' +
    '  width:min(440px,78vw,62vh);height:min(440px,78vw,62vh);z-index:1000}' +
    '.scrim{display:none}' +
    ':host([data-reframe]) .scrim{display:block;position:fixed;inset:0;z-index:-1;' +
    '  background:rgba(0,0,0,.32);backdrop-filter:saturate(140%) blur(14px);' +
    '  -webkit-backdrop-filter:saturate(140%) blur(14px)}' +
    '.hint{display:none}' +
    ':host([data-reframe]) .hint{display:block;position:absolute;left:50%;top:100%;' +
    '  transform:translateX(-50%);margin-top:18px;white-space:nowrap;' +
    '  padding:8px 15px;border-radius:980px;background:rgba(255,255,255,.16);' +
    '  backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);color:#fff;' +
    '  font:400 13px/1 -apple-system,BlinkMacSystemFont,"SF Pro Text",sans-serif;' +
    '  letter-spacing:-.08px;pointer-events:none}' +
    ':host([data-reframe]) .frame{box-shadow:0 0 0 2px var(--ac,#0071e3)}' +
    // Crop guideline: the mask's own outline (a 1:1 circle for the avatar slot)
    // drawn over the spill so you can see exactly what will remain.
    ':host([data-reframe]) .frame::after{content:"";position:absolute;inset:0;' +
    '  border-radius:inherit;border:1.5px dashed var(--ac,#0071e3);box-sizing:border-box;' +
    '  box-shadow:0 0 0 1px rgba(0,0,0,.28),inset 0 0 0 1px rgba(0,0,0,.28);' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        '<div class="scrim" part="scrim" data-dc-edit-transparent></div>' +
        ...
        '<div class="hint" part="hint" data-dc-edit-transparent>Drag to reposition · Scroll to zoom · Esc when done</div>' +
        '<input type="file" accept="' + ACCEPT.join(',') + '" hidden>';
      this._frame = root.querySelector('.frame');
      this._scrim = root.querySelector('.scrim');
      this._scrim.addEventListener('pointerdown', (e) => {
        e.preventDefault();
        e.stopPropagation();
